...
void TFG_Surf_condition_write(double **values,int NX, int NY, int NZ, double xcorner, double ycorner, double zcorner, double xres, double yres, double zres, char *input_name){
...
void OG_Surf_condition_write(double **top, double **values,int NX, int NY, int NZ, double xcorner, double ycorner, double zcorner, double xres, double yres, double zres, char *input_name){
		...
		// write values file   
	        ...
	        double *svalues = (double*)malloc(nx[0]*ny[0]*nz[0]*sizeof(double));        
                for(k=0;k<NZ;k++){
	              ...
	              for(j=0;j<NX;j++){	   
	                count=count+1;
	                if(((k*zres)<=top[i][j]) && ((k*zres)>=(top[i][j]-zres))){         
	                  svalues[count] = values[i][j];
	                }
	                else{
	                  svalues[count] = 0.00;
...
//        if (idx==-9999) {return -1;} // Error so bail
  ...
  if (AllPatches[0].patch_cell_count>0) {printf(" Bottom "); patch_ORDER = "Bottom ";}
  if (AllPatches[1].patch_cell_count>0) {printf(" Top "); patch_ORDER = concat(patch_ORDER, " Top ");}

  if (AllPatches[2].patch_cell_count>0) {printf(" Left "); patch_ORDER = concat(patch_ORDER, " Left ");}  // West
  if (AllPatches[3].patch_cell_count>0) {printf(" Right "); patch_ORDER = concat(patch_ORDER, " Right ");} // East

  if (AllPatches[4].patch_cell_count>0) {printf(" Front "); patch_ORDER = concat(patch_ORDER, " Front ");} // South
  if (AllPatches[5].patch_cell_count>0) {printf(" Back "); patch_ORDER = concat(patch_ORDER, " Back");}  // North
  for (i=6; i<(6+np_usr+1); ++i)
  {
    if (sub_patches==1) {
      if ((AllPatches[i].patch_cell_count>0)&(AllPatches[i].value!=0))
      {
        ...
        if (AllPatches[i].act_faces[j]>0)
           if(AllPatches[i].value == -7){ //added by Tomas
              printf(" Usr_Top01_%s ",dir_labels[j]);              
              patch_ORDER = concat(patch_ORDER," Usr_");
              patch_ORDER = concat(patch_ORDER, "Top01");
              patch_ORDER = concat(patch_ORDER, "_");
              patch_ORDER = concat(patch_ORDER, dir_labels[j]);
           } 
           else{
              sprintf(allpatches,"%d",AllPatches[i].value);//added by Tomas
              printf(" Usr_%i_%s ",AllPatches[i].value,dir_labels[j]);
              patch_ORDER = concat(patch_ORDER," Usr_");
              patch_ORDER = concat(patch_ORDER, allpatches);
              patch_ORDER = concat(patch_ORDER, "_");
              patch_ORDER = concat(patch_ORDER, dir_labels[j]);
           }           
        }
    ...
    {  
       if(AllPatches[i].value == -7){ //added by Tomas      
       printf(" Usr_Top01 ");
       patch_ORDER = concat(patch_ORDER," Usr_Top01");
       } 
       else{
       sprintf(allpatches, "%d", AllPatches[i].value);// added by Tomas
...
//  double *DTd;
  double *DTf;  
  DTf = (double*)calloc(nxyzp,sizeof(double));
//  DTf = (double*)malloc(nxyzp*sizeof(double));    
  for (i = 0; i < NY; ++i){
     for (j = 0; j < NX; ++j){       
         DTf[countn] = values[i][j];
         countn = countn+1;
     }
  } 
  ...
  tools_WriteDouble(fp, DTf, NX*NY*NZ);

//  endian_swap_fwrite(DTf, sizeof(double), NX*NY*NZ, fp);
  free(DTf);
